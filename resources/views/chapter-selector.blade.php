...
@section('main-content')
  <div class="box box-primary">
    ...
    <form role="form" method="POST" action="{{url('chapterupdateformpost')}}" enctype="multipart/form-data" >
        {{csrf_field()}}
      <div class="box-body">
        @if(count($errors))
      ...
      <div class="box-footer">
        <button type="button" class="btn btn-info" data-toggle="modal" data-target="#modal-info">Actualizar</button>
  ...
  <div class="box box-danger">
    ...
    <form role="form" method="POST" action="{{url('chapterdeletepost')}}" enctype="multipart/form-data" >
        {{csrf_field()}}
      <div class="box-body">
        @if(count($errors))
        <div class="alert alert-danger container-fluid">
      ...
      @endif
        ...
        <div class="form-group{{ $errors->has('history_id') ? ' has-error' : '' }}">
          <label>Historia</label>
          <select class="form-control select2" style="width: 100%;" name="chapter_id" id="category">
            @foreach($chapters as $chapter)
            <option>{{ $chapter->id.'-Capitulo:'.$chapter->chapter }}</option>
            @endforeach
          </select>
        </div>
